...
export default function App() {
  ...
  const fetchGates = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/gates`);
  ...
  const fetchSystemMode = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/gates/settings`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/reports/stats`);
      const data = await res.json();
      setStats(data);
    } catch (e) {
      console.error(e);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/users/login`, {
        method: 'POST',
  ...
  const handleLogout = async () => {
    if (currentUser) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/users/logout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
