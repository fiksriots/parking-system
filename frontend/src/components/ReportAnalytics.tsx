...
export const ReportAnalytics: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      // 1. Fetch stats
      const statsRes = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/reports/stats`);
      const statsData = await statsRes.json();
      setStats(statsData);

      // 2. Fetch hourly stats
      const hourlyRes = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/reports/hourly-stats`);
      const hourlyData = await hourlyRes.json();
      setHourlyData(hourlyData);

      // 3. Fetch recent logs
      const logsRes = await fetch(`${import.meta.env.VITE_API_URL || window.location.origin}/api/reports/recent-logs`);
