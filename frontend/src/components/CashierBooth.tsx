...
export const CashierBooth: React.FC<CashierBoothProps> = ({ socket, gates, systemMode = 'simulation', currentUser }) => {
  ...
  const fetchActiveShift = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/users/shift/active/${currentUser.username}`);
      if (res.ok) {
  ...
  const handleOpenShift = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:3000/api/users/shift/open', {
        method: 'POST',
  ...
  const loadShiftSummary = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/users/shift/summary/${activeShift.id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchShiftTransactions = async (shiftId?: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/users/shift/transactions/${id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCloseShift = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:3000/api/users/shift/close', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleSubmitPOS = async () => {
    ...
    if (checkoutData.isLostTicket) {
      ...
      try {
        const res = await fetch('http://localhost:3000/api/payment/lost-ticket', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            gateId: selectedGateId,
